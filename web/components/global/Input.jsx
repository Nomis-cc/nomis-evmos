...
export default function Input({ fullAddress, blockchain }) {
  const initialInput = getBlockchainId("evmos");
  ...
  const handleClick = async () => {
    setIsLoading(true);
    await router.push(`/score/${blockchains[active].slug}/${address}`);
  };
  ...
  return (
    <div className="Input">
      ...
      >
        <li className="blockchainActive" onClick={() => setIsOpen(!isOpen)} ref={ref}>
